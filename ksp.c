...
int main(int argc,char **args)
{
  ...
  */
  ...
  ierr = PetscOptionsGetInt
    (
     ...
     NULL,
#endif
     PETSC_NULL,"-n",&domain_size,&flag); CHKERRQ(ierr);
  if (!flag) domain_size = 100;
  ...
  ierr = PetscOptionsGetReal
    (
     ...
     NULL,
#endif
     PETSC_NULL,"-unsymmetry",&unsymmetry,&flag); CHKERRQ(ierr);
  ...
  */
  ...
  ierr = KSPSetOperators(Solve,A,A); CHKERRQ(ierr);
